...
// ════════════════════════════════════════════════════════════════════════════
// CHANGELOG v22.12 — June 2026
// ════════════════════════════════════════════════════════════════════════════
//
// BUG A (CRITICAL) — Notifications page stuck in loop on same trader's post
// ─────────────────────────────────────────────────────────────────────────────
// SYMPTOM: Script keeps navigating back to the same /post/ page repeatedly
//          (e.g. Normie BTC), stays there 22-27 seconds per attempt, returns
//          to /notifications, then immediately goes back to the same post again.
//
// ROOT CAUSE — pendingTrades stale age:
//   pendingTrades entries are saved to localStorage with `age` = the ORIGINAL
//   age at first detection (e.g. `age: 2` for "opened 2 minutes ago").
//   On every page reload, `loadPendingTrades()` restores these entries from
//   localStorage. The freshness filter `t.age <= MAX_AGE_MINUTES` sees `age=2`
//   and keeps the trade as "fresh" — even if it was saved 1 hour ago.
//   The script dutifully navigates to the post, times out finding Mimic Trade
//   (the trade is now an "Updated Position" post, not a fresh open), calls
//   `seenNotifs.delete(key)` to allow a retry, returns to notifications, and
//   the entire cycle repeats because pendingTrades STILL has the stale entry.
//
// FIX 1 — Effective age calculation on pendingTrades filter (line in scanNotifs):
//   Each pendingTrade entry has `savedAt = Date.now()` set when first queued.
//   Effective age = original_age + (now - savedAt) / 60000.
//   Example: trade originally detected at 2min old, saved 58 minutes ago.
//   Effective age = 2 + 58 = 60 minutes > MAX_AGE_MINUTES(10) → FILTERED OUT.
//   This purge runs at the top of every scanNotifs() call.
//
// FIX 2 — seenNotifs guard on pendingTrades target selection:
//   After doOpen() times out on attempt 1, it calls `seenNotifs.delete(key)`.
//   Then unlock() returns to notifications. Next scan: addTrade() sees the
//   notification at age>10 → adds to seenNotifs as STALE. But pendingTrades
//   still has the entry. Without this guard, `pendingTrades.shift()` gives back
//   the same stale trade — key is now in seenNotifs but pendingTrades ignores it.
//   Fix: before using a pendingTrades target, verify `!seenNotifs.has(key)`.
//   If already marked stale, skip and continue to next pending or fresh trade.
//
// BUG B — seenWallet compositeKey expires in 4 seconds (too short)
// ─────────────────────────────────────────────────────────────────────────────
// SYMPTOM: Wallet page processes Trade Updates incompletely or starts
//          duplicate processGroupQueue runs.
//
// ROOT CAUSE:
//   `handleWalletTradeUpdates()` used `setTimeout(()=>seenWallet.delete(key), 4000)`
//   to expire the composite dedup key after 4 seconds. But a single
//   `processSingleCopy` cycle takes: CONFIRM_WAIT_MS(3.5s) + pollForModal(up to
//   10s) + afterConfirm verification(up to 3s) = potentially 16 seconds per Copy.
//   If there are 2+ Copy buttons, processGroupQueue runs for 32+ seconds total.
//   After 4 seconds the key is gone. The next scanWallet() tick calls
//   handleWalletTradeUpdates() again → new compositeKey → new lock() → new
//   processGroupQueue() → BUT isExec=true → scanWallet() returns early (safe).
//   HOWEVER: the sweep callback (line after runWalletScrollSweep) also calls
//   handleWalletTradeUpdates() after the sweep completes, and if isExec was
//   just cleared by unlock(), this could start a second queue concurrently.
//
// FIX: Increased compositeKey TTL from 4,000ms → 60,000ms (60 seconds).
//      This safely covers the longest possible multi-group copy session.
//
// NOTE on 1115% vs 19.41%:
//   NOT a bug. When Normie increased his BTC position by 19.41%, that 19.41%
//   is relative to his own large existing position. Your mimic position was
//   opened much smaller. Involio calculates the equivalent adjustment for your
//   account and displays it as +1,115.8% relative to YOUR original mimic size.
//   The script confirmed it correctly. The wallet display is Involio's math.
//
// ════════════════════════════════════════════════════════════════════════════
const OPEN_PHRASES       = [
