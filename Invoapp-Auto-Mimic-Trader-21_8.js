...
function handleWalletTradeUpdates() {
  ...
  // ★ v21.8: Composite key now includes the current number of visible Copy buttons.
  // PROBLEM: "Trade Updates (1)" before a close AND after a close have IDENTICAL
  // label text — same count number, just different row content inside. The old key
  // was seenWallet for 2 minutes, so the close update was silently skipped for
  // up to 2 minutes after appearing.
  //
  // FIX 1: Include findVisibleCopyButtons().length in the composite key.
  //   Before close: 0 Copy buttons (position increase row already Copied)
  //   After close appears: 1 Copy button (close row needs to be Copied)
  //   → key changes → re-triggers immediately
  //
  // FIX 2: Expiry reduced from 2 minutes to 20 seconds.
  //   The page reloads every 15-45s. A 20s expiry prevents double-processing
  //   within the same page load, but guarantees that after a reload a fresh
  //   copy of the same group label is always processed.
  const pendingCopyCount = findVisibleCopyButtons().length;
  const compositeKey = 'wallet-groups::' + groups.map(g => g.label).sort().join('|') + `::copies=${pendingCopyCount}`;
  if (seenWallet.has(compositeKey)) return false;
  seenWallet.add(compositeKey);
  // ★ v21.8: was 120000 (2 min) — reduced to 20s to match reload cycle
