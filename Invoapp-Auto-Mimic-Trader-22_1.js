...
function scanWallet() {
  ...
  // 4. CLOSE POSITION — trader has closed; act immediately, never blocked
  //    by Trade Updates (FIX v22.1: promoted from priority 7 to priority 4)
  // 5. Dollar stop-loss — auto-close any trade exceeding $ threshold
  // 6. Emergency alert — large loss crossing both % AND $ thresholds
  // 7. Trade Updates — process all pending Copy buttons across all trades

  verifyPostOpenRiskIfNeeded();

  // FIX v22.1: Close Position is checked FIRST — before Trade Updates.
  // A trader closing their position is the highest-urgency event on the
  // wallet page and must never be blocked by Trade Update group expansion.
  if (checkClosePosition()) return;

  // Dollar stop-loss — runs before the emergency check since it's gentler
  // and more common (closes one trade at a fixed $ loss).
  if (checkDollarStopLoss()) return;

  // Emergency alert — by default requires BOTH the % and $ thresholds to be
  // breached on the same trade (EMERGENCY_REQUIRES_BOTH), which avoids false
  // alarms on high-leverage positions that show a big % move on tiny $ size.
  ...
  // Trade Updates run last — they take the longest (expand groups, click Copy,
  // wait for modals). Since Close Position and emergency checks now run first,
  // Trade Updates can no longer block urgent close actions.
