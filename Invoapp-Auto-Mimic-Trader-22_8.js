...
const AUTO_CLOSE_USD_THRESHOLD  = 7.5;

// ── EMERGENCY ALERT ─────────────────────────────────────────────────────────
const EMERGENCY_WALLET_LOSS_PCT = 20.0;
const EMERGENCY_MAX_LOSS_USD    = 80.0;
...
// Tracks every position the script has opened so wallet tab can verify
// that a Close Position action belongs to one of our mimicked trades.
// Registry entry: { symbol, trader, leverage, direction, openedAt }
// ════════════════════════════════════════════════════════════════════════════
...
function isMimicPosition(symbol) {
  ...
  const positions = loadOpenMimicPositions();
  if (positions.length === 0) return true; // empty registry = no info = trust the signal
  return positions.some(p => p.symbol === symbol);
}
function getMimicPositionDirection(symbol) {
...
function loadPendingTrades() {
  try {
    ...
    if (restored > 0) { L(`loadPendingTrades: restored ${restored} pending trade(s)`); logAct('PENDING_RESTORED', `${restored} trade(s) re-queued`); }
  } catch {}
}
...
window._status = () => {
  ...
  console.log(`Stop loss: ${MANAGE_STOP_LOSS ? `${STOP_LOSS_PCT}%` : 'OFF'}`);
  console.log(`Dollar stop-loss: ${ENABLE_DOLLAR_STOP_LOSS ? `ACTIVE — $${AUTO_CLOSE_USD_THRESHOLD}` : 'DISABLED'}`);
  console.log(`Emergency: ${EMERGENCY_WALLET_LOSS_PCT}% AND $${EMERGENCY_MAX_LOSS_USD}`);
  console.log(`Urgent close: ${urgentCloseSignalTs ? `ACTIVE (${Math.round((Date.now() - urgentCloseSignalTs)/1000)}s ago) symbol=${urgentCloseSymbol||'?'}` : 'none'}`);
  console.log(`Sweep: ${walletScrollSweepActive} | openAttempts: ${JSON.stringify(Object.fromEntries(openAttempts))}`);
...
window._unpause          = () => { openPauseUntil = 0; setRiskState({ openPauseUntil: 0 }); clearEmergencyBanner(); badge('🟢 Active'); L('Open pause cleared'); };
window._pauseOpens       = (min=30, reason='manual') => setOpenPause(min*60000, reason);
window._clearSeen        = () => { seenNotifs = new Set(); openAttempts.clear(); saveSeenNotifs(); L('seenNotifs + openAttempts cleared'); };
window._clearMimicRegistry = () => { saveOpenMimicPositions([]); L('Mimic registry cleared'); };
...
function badge(text, color = '#16a34a') {
  ...
  if (!b) {
    ...
    b.title = 'Click for log | _status() | _pauseOpens(min) | _unpause() | _clearMimicRegistry() | _addMimicPosition(sym,dir,trader)';
...
function unlock() {
  ...
  if (IS_WALLET && urgentCloseSignalTs && Date.now() - urgentCloseSignalTs < URGENT_CLOSE_SIGNAL_TTL) {
    L('unlock: close signal still hot — immediate re-scan');
    logAct('UNLOCK_RESCAN', 'Close signal hot — rescanning');
...
// v22.6: stableKey includes creation-time bucket so same notification aging
// across reloads gets the same key but different notifications get different keys.
...
function readWalletTotal() {
  ...
  try {
    ...
    if (stored.balance>1 && Date.now()-(stored.ts||0)<30*60000) { L(`readWalletTotal: stored $${stored.balance}`); return stored.balance; }
  } catch {}
  return null;
}
...
// v22.7/22.8: Tighter 120-char window + registry filter
...
// v22.6: storeWalletBalance with near-Today strategy
function storeWalletBalance() {
  ...
  if (!bal||bal<10) {
    ...
    if (todayIdx>0) {
      ...
      if (amounts.length) { bal=Math.max(...amounts); source='near-Today max'; }
    }
  }
  if (!bal||bal<10) { L('storeWalletBalance: not found'); return; }
  try {
...
function verifyStopLossVisible(expectedStr, done, attempt=0) {
  ...
  const ok=(body.includes('Stop-Loss Price')||body.includes('Stop Loss Price')||body.includes('Stop-Loss'))&&(body.includes(expectedStr)||body.includes(norm)||!!findContaining(expectedStr));
...
function scanNotifs() {
  ...
  const addTrade = (el,label) => {
    ...
    const age=ageMins(label);
    if (age>MAX_AGE_MINUTES) { seenNotifs.add(key); saveSeenNotifs(); if (age<30) logAct('STALE',`${age.toFixed(1)}m: ${label.substring(0,60)}`); return; }
  ...
  if (!target.el) {
    ...
    if (!target.el) { W(`pendingTrades: cannot re-find element — skipping`); savePendingTrades(); setTimeout(scanNotifs,800); return; }
  }
...
function doOpen(tradeTarget=null) {
  ...
  waitForPredicate(
    ...
    (mimicBtn)=>{
      ...
      setTimeout(()=>{
        setPositionSize((sizeOK,sizeCtx)=>{
          if (!sizeOK) { W('Size verify failed — proceeding'); logAct('SIZE_WARN','Proceeding without verified size'); }
          setTimeout(()=>{
            setStopLoss((slOK,slCtx)=>{
              ...
              setTimeout(()=>{
                waitForEl(['Confirm Position','Confirm'],()=>{
                  ...
                  logAct('SUCCESS',`Trade opened ${openCtx.symbol||''} ${openCtx.direction||''} ${new Date().toLocaleTimeString()}`);
                  ...
                  // v22.7/22.8: Register opened position
                  if (openCtx.symbol) {
    ...
    ()=>{
      ...
      if (attempts<MAX_OPEN_ATTEMPTS&&key) { seenNotifs.delete(key); saveSeenNotifs(); W(`doOpen: retry ${attempts}/${MAX_OPEN_ATTEMPTS}`); logAct('RETRY_OPEN',`${attempts}/${MAX_OPEN_ATTEMPTS}: ${key.substring(0,50)}`); badge(`⏭ RETRY ${attempts}/${MAX_OPEN_ATTEMPTS}`,'#b45309'); }
      else { W(`doOpen: exhausted attempts`); logAct('SKIP_PERM',`${attempts} attempts: ${key?.substring(0,50)}`); badge('⏭ NO MIMIC BTN','#b45309'); }
      unlock();
    }
  );
}
...
function doCloseConfirm(attempt=0, symbol=null, direction=null) {
  ...
  if (btn&&modal) { click(btn); lastActionTime=Date.now(); logAct('CLOSED',`${symbol||''} at ${new Date().toLocaleTimeString()}`); clearEmergencyBanner(); setTimeout(()=>closeDone(symbol,direction),CONFIRM_WAIT_MS); }
...
function expandTradeUpdateGroup(group) {
  if (!group.el||!visible(group.el)) { W(`expandTradeUpdateGroup: "${group.label}" not visible`); return false; }
...
// ════════════════════════════════════════════════════════════════════════════
// v22.7 FIX (retained): detectModalType — safer close modal detection.
// v22.8 FIX (new): detectDirectionFlip — identifies when a Trade Update
//   copy modal is actually a DIRECTION FLIP (Long→Short or Short→Long).
//
// ROOT CAUSE of BTC -$10.30 false close:
// When the trader flipped BTC from Long to Short, Involio sent a "Trade Update"
// notification. The script correctly clicked Copy, and a confirm modal appeared.
// The script detected it as a standard 'confirm' type and clicked Confirm —
// which on Involio means: CLOSE the existing Long, OPEN a new Short.
// This is correct behavior for a direction flip, BUT the problem is the script
// didn't LOG or TRACK this registry update — so when the close happened it
// wasn't understood as "we intentionally closed our Long to flip to Short",
// it just appeared as a surprise loss in the wallet history.
//
// Actually the deeper remaining risk: if a Copy modal appears that would CLOSE
// a position (e.g. "Reduce to 0%" or a full position exit trade update), the
// script confirms it without knowing it's a closing action.
//
// v22.8 FIX: detectDirectionFlip() reads the modal body text for phrases that
// indicate a Long→Short or Short→Long change. If detected:
//   1. The old direction is unregistered from the mimic registry.
//   2. The new direction is registered.
//   3. The action is prominently logged so you can see it in _status().
//   4. If the flip would result in CLOSING our position in a direction the
//      registry shows as currently open for that symbol, we allow it (correct
//      trader action) but log it clearly as a FLIP, not a surprise close.
// ════════════════════════════════════════════════════════════════════════════

function detectDirectionFlip(modalBodyText) {
  const b = modalBodyText || document.body.innerText || '';
  // Look for explicit flip signals in the modal text
  ...
  const hasFlipPhrase = /flip|switch|reverse|direction/i.test(b);
  ...
  if ((hasCloseLong && hasOpenShort) || (hasCloseShort && hasOpenLong) || hasFlipPhrase) {
    return { isFlip: true, symbol, newDirection, oldDirection: newDirection==='short'?'long':'short' };
  }
  // Softer heuristic: modal contains both Long and Short words for the same symbol
  if (symbol && /\bLong\b/i.test(b) && /\bShort\b/i.test(b)) {
    return { isFlip: true, symbol, newDirection, oldDirection: newDirection==='short'?'long':'short' };
  }
  return { isFlip: false, symbol, newDirection, oldDirection: null };
}
...
function detectModalType() {
  ...
  // v22.7: Close modal requires explicit close signal AND no position-size input
  if (hasExitPrice&&hasConfirm) {
    const isGenuineClose=(hasClosePos||hasCloseTrade)&&!hasPositionSz;
    if (isGenuineClose) return 'close';
    L(`detectModalType: Exit Price+Confirm but not genuine close — hasClosePos=${hasClosePos} hasPositionSz=${hasPositionSz}`);
    return hasPositionSz?'adjust':'adjust';
  }
...
function processSingleCopy(loop, maxLoops, groupEl, onDone) {
  ...
  function afterConfirm() {
    ...
    const verifyCopied=()=>{
      ...
      if (nowCopies<copies.length||nowCopied>doneSoFar) { logAct('COPY_VERIFIED','Copied confirmed'); setTimeout(()=>processSingleCopy(loop+1,maxLoops,groupEl,onDone),CONFIRM_WAIT_MS); }
      else if (++v>=VERIFY_MAX) { W(`Copy→Copied unconfirmed — proceeding`); setTimeout(()=>processSingleCopy(loop+1,maxLoops,groupEl,onDone),CONFIRM_WAIT_MS); }
  ...
  function pollForModal() {
    ...
    if (!modalType) {
      ...
      if (modalElapsed>=MODAL_MAX_WAIT) { W(`No modal after ${MODAL_MAX_WAIT}ms — skip loop ${loop}`); logAct('COPY_NO_MODAL',`Loop ${loop} timeout`); setTimeout(()=>processSingleCopy(loop+1,maxLoops,groupEl,onDone),500); }
    ...
    // ════════════════════════════════════════════════════════════════════════
    // v22.8 KEY FIX: Before confirming ANY modal during a Copy loop,
    // check if it represents a direction flip (Long→Short / Short→Long).
    // If it does, update the registry BEFORE confirming so the system
    // correctly tracks the new direction of the position.
    //
    // This prevents the registry from showing a stale direction after a flip,
    // which was causing the wallet tab to miss-identify future close signals.
    // ════════════════════════════════════════════════════════════════════════
    const bodyNow = document.body.innerText || '';
    const flipInfo = detectDirectionFlip(bodyNow);

    if (flipInfo.isFlip && flipInfo.symbol) {
      W(`processSingleCopy: 🔄 DIRECTION FLIP detected — ${flipInfo.symbol} ${flipInfo.oldDirection}→${flipInfo.newDirection}`);
      logAct('FLIP_DETECTED', `${flipInfo.symbol}: ${flipInfo.oldDirection}→${flipInfo.newDirection} — updating registry`);
      // Unregister the old direction, register the new one
      if (flipInfo.oldDirection) unregisterOpenMimicPosition(flipInfo.symbol, flipInfo.oldDirection);
      if (flipInfo.newDirection) registerOpenMimicPosition(flipInfo.symbol, '', 0, flipInfo.newDirection);
    }
    ...
    // v22.7 retained: close modal mid-Copy loop is always suspect — dismiss safely
    else if (modalType==='close') {
      W(`processSingleCopy: 'close' modal mid-Copy — REFUSING to confirm (safety guard)`);
      logAct('COPY_CLOSE_GUARD',`Loop ${loop}: close modal during Trade Update — dismissing`);
      dismissMenu();
...
function processGroupQueue(groups, groupIdx) {
  if (groupIdx>=groups.length) {
    ...
    } else { logAct('UPDATED',`All ${groups.length} group(s) done — ${alreadyCopied} Copied`); unlock(); }
    return;
  }
...
function handleWalletTradeUpdates() {
  ...
  if (pendingCopyCount>0) { lock('UPDATING'); logAct('UPDATING',`${groups.length} group(s) | ${pendingCopyCount} Copy pending`); processGroupQueue(groups,0); return true; }
...
function walletChecksAtCurrentPosition() {
  ...
  if (worstTrade) {
    ...
    if (triggered) { attemptEmergencyClose(`Trade ${worstTrade.symbol} ${worstTrade.pct??'?'}% / -$${worstTrade.usd?.toFixed(2)??'?'}`); return true; }
  }
  return false;
}
function runWalletScrollSweep(onActionFired, onComplete) {
  ...
  logAct('SCROLL_SWEEP',`${isUrgent?'URGENT':'normal'} — ${pauseMs}ms/pos | closeSignal=${isUrgent}`);
  L(`runWalletScrollSweep: ${isUrgent?'🔴 URGENT (Trade Updates suspended)':'normal'} — ${pauseMs}ms/pos`);
  const maxSweepMs=WALLET_SCROLL_MAX_POS*Math.max(pauseMs,150)+5000;
  const safetyTimer=setTimeout(()=>{ if (walletScrollSweepActive) { W(`Sweep safety timeout`); logAct('SWEEP_TIMEOUT','Safety reset'); walletScrollSweepActive=false; onComplete?.(); } },maxSweepMs);
  flutterScrollToTop().then(()=>{
    ...
    function step() {
      try {
        ...
        if (walletChecksAtCurrentPosition()) { logAct('SCROLL_FOUND',`Action at pos ${pos}`); clearTimeout(safetyTimer); walletScrollSweepActive=false; onActionFired?.(); return; }
        const closeSignalHotNow=urgentCloseSignalTs&&Date.now()-urgentCloseSignalTs<URGENT_CLOSE_SIGNAL_TTL;
        if (!closeSignalHotNow&&!isExec&&handleWalletTradeUpdates()) { logAct('SCROLL_UPDATES',`Trade Updates at pos ${pos}`); clearTimeout(safetyTimer); walletScrollSweepActive=false; onActionFired?.(); return; }
        for (let i=0;i<WALLET_SCROLL_STEPS;i++) flutterWheel(300);
        pos++;
        setTimeout(step,pauseMs);
      } catch(e) { W(`sweep step error at pos ${pos}: ${e.message}`); logAct('SWEEP_ERROR',`pos ${pos}: ${e.message}`); clearTimeout(safetyTimer); walletScrollSweepActive=false; onComplete?.(); }
    }
    step();
  });
}

// ── checkClosePosition with symbol verification (v22.7 + v22.8 retained) ─────
function readSymbolFromCloseButton(btn) {
  ...
  const window200=body.substring(Math.max(0,closeIdx-200),closeIdx+50);
  const m2=window200.match(/\b([A-Z]{2,10})\s+\d+\s*[Xx]\s+(Long|Short)\b/);
...
function checkClosePosition() {
  ...
  if (hasTraderClosed) {
    ...
    if (!hasClosePos) { L(`checkClosePosition: "Trader Closed" badge — urgent mode ON`); logAct('TRADER_CLOSED_BADGE','Urgent mode active'); return false; }
  }
  try {
    const lastCloseTs=parseInt(localStorage.getItem(CLOSE_POSITION_TS_KEY)||'0',10);
    if (lastCloseTs&&Date.now()-lastCloseTs<CLOSE_POSITION_COOLDOWN_MS) { L(`checkClosePosition: cooldown active`); return false; }
  } catch {}
  if (menuOpen()) return false;
  const btn=findByText('Close Position')||findContaining('Close Position');
  if (!btn) {
    urgentCloseSignalTs=Date.now();
    L(`checkClosePosition: 🔴 Close Position in body but off-screen — urgent sweep`);
    logAct('CLOSE_SIGNAL_OFFSCREEN','Urgent sweep activated');
    return false;
  }
  // v22.7/22.8: Verify symbol is in mimic registry before acting
  ...
  if (symbol) {
    if (!isMimicPosition(symbol)) {
      W(`checkClosePosition: ⚠️ SKIP — ${symbol} NOT in mimic registry (foreign/manual position)`);
      logAct('CLOSE_GUARD',`Skipped ${symbol} — not in registry`);
      return false;
    }
    L(`checkClosePosition: ✅ ${symbol} in registry — proceeding`);
    urgentCloseSymbol=symbol;
  } else {
    L(`checkClosePosition: symbol unreadable — proceeding (registry: ${loadOpenMimicPositions().length} entries)`);
  }
  urgentCloseSignalTs=Date.now();
  ...
  L(`checkClosePosition: 🟢 VISIBLE — clicking NOW (${symbol||'?'})`);
  lock('CLOSING');
  click(btn);
...
function scanWallet() {
  ...
  if (worstTrade) {
    ...
    const usdBreach=worstTrade.usd!==null&&worstTrade.usd>=EMERGENCY_MAX_LOSS_USD;
    const triggered=EMERGENCY_REQUIRES_BOTH?(pctBreach&&usdBreach):(pctBreach||usdBreach);
    if (triggered) { attemptEmergencyClose(`Trade ${worstTrade.symbol} ${worstTrade.pct??'?'}% / -$${worstTrade.usd?.toFixed(2)??'?'}`); return; }
  }
  if (!closeSignalHot&&handleWalletTradeUpdates()) return;
  if (closeSignalHot) L(`scanWallet: close signal hot — URGENT sweep`);
...
// ── WATCHDOGS ──────────────────────────────────────────────────────────────────
setInterval(()=>{
  if (isExec&&execStart&&Date.now()-execStart>MAX_EXEC_MS) { W(`Stuck ${MAX_EXEC_MS/1000}s — force-unlock`); isExec=false; execStart=0; badge('🟢 Active'); window.location.href=HOME_URL; }
},10000);
setInterval(()=>{
...
setInterval(()=>{
  ...
  if (is404&&!window._404recovering) { window._404recovering=true; logAct('404','Recovering'); badge('⚠️ 404 recovering','#dc2626'); setTimeout(()=>{ window._404recovering=false; window.location.href=HOME_URL; },1500); }
...
function startAuthWatchdog() {
  ...
  function handleAuthFailure(url) {
    ...
    failCount++;
    W(`AUTH WATCHDOG: 401 (${failCount}/${SESSION_RELOAD_THRESHOLD})`);
    ...
    if (failCount>=SESSION_RELOAD_THRESHOLD&&!reloadPending) {
      reloadPending=true; badge('🔄 Session expired','#b45309'); logAct('AUTH_RELOAD','Session expired');
...
function startWebLocksKeepalive() {
  if (!navigator.locks?.request) { W('Web Locks API not available'); return; }
  navigator.locks.request('am-tab-alive',{mode:'shared'},()=>new Promise(()=>{})).catch(e=>W(`Web Locks error: ${e.message}`));
  L('Web Locks keepalive: ACTIVE');
}
...
function startMutationWatcher() {
  ...
  const runScan=()=>{ lastRun=Date.now(); if (isExec||!onPage('/notifications')||opensPaused()) return; try { scanNotifs(); } catch(e) { W(`mutation scan error: ${e.message}`); } };
  ...
  L('Notifications MutationObserver: ACTIVE (250ms throttle)');
}
function startWalletMutationWatcher() {
  ...
  const runCheck=()=>{
    ...
    L('Wallet MutationObserver: signal — scanWallet()');
    try { scanWallet(); } catch(e) { W(`wallet mutation scan error: ${e.message}`); }
  };
  const observer=new MutationObserver(()=>{
    const now=Date.now();
  ...
  observer.observe(target,{childList:true,subtree:true,attributes:false,characterData:false});
  L('Wallet MutationObserver: ACTIVE (200ms throttle)');
}
...
function startAntiThrottleWorker() {
  ...
  } catch(e) { W(`Anti-throttle worker failed: ${e.message}`); }
}
...
try {
  const lastCloseTs=parseInt(localStorage.getItem(CLOSE_POSITION_TS_KEY)||'0',10);
  if (lastCloseTs&&Date.now()-lastCloseTs>CLOSE_POSITION_COOLDOWN_MS) { localStorage.removeItem(CLOSE_POSITION_TS_KEY); L('Boot: stale CLOSE_POSITION_TS_KEY cleared'); }
  else if (lastCloseTs) L(`Boot: close cooldown active (${Math.round((CLOSE_POSITION_COOLDOWN_MS-(Date.now()-lastCloseTs))/1000)}s remaining)`);
} catch {}

{
  const positions=loadOpenMimicPositions();
  L(`Boot: mimic registry — ${positions.length} open: ${positions.map(p=>`${p.symbol}/${p.direction}`).join(', ')||'none'}`);
}
...
setTimeout(()=>{
  ...
  if (IS_WALLET) {
    setInterval(()=>{
      ...
      if (!isExec) {
        const copies=findVisibleCopyButtons();
        if (copies.length>0) {
          logAct('URGENT_COPY',`Urgent: ${copies.length} Copy visible`);
  ...
  setTimeout(()=>{
    ...
    L(`v22.8 FIXES (all prior fixes retained):`);
    L(`  FIX 6 — detectDirectionFlip(): reads modal body during every Copy loop confirm`);
    L(`           Detects Long→Short / Short→Long flips BEFORE confirming`);
    L(`           Updates mimic registry atomically: removes old dir, adds new dir`);
    L(`           Logs FLIP_DETECTED so you can see direction changes in _status()`);
    L(`           This is why BTC -$10.30 happened: flip was confirmed as a Copy modal`);
    L(`           but the registry wasn't updated, so future close signals were misread`);
    L(`  FIX 1 — detectModalType(): 'close' requires "Close Position"/"Close Trade" + no PositionSize`);
    L(`  FIX 2 — findWorstTradeCard(): 120-char window + registry filter`);
    L(`  FIX 3 — checkClosePosition(): symbol verified against registry before acting`);
    L(`  FIX 4 — processSingleCopy(): 'close' modal mid-Copy → refuse + dismiss`);
    L(`  FIX 5 — Open mimic registry: tracks all opened positions across tabs`);
    L(`Auth watchdog: ${SESSION_WATCHDOG_ENABLED?`ACTIVE (${SESSION_RELOAD_THRESHOLD} consecutive 401s)`:'DISABLED'} | Web Locks: ACTIVE`);
    L(`Commands: _status() | _pauseOpens(min) | _unpause() | _clearSeen() | _clearMimicRegistry() | _addMimicPosition(sym,dir,trader)`);
  },2200);
},1000);
