...
// ── RELOAD TIMING ──────────────────────────────────────────────────────────
// Each tab independently reloads on a random delay between these two values
// (inclusive). DO NOT set above 60s — trades will be missed.
...
// FIX v22.1: Reduced from 20min to 8min. 20min was the hidden source of the
// "30-minute delay" — if lastActivity wasn't refreshed for any reason, the tab
// would sit doing nothing for up to 20 minutes before force-reloading.
...
// FIX v22.1: How long a "Close Position" detection is remembered (in ms).
// v22.0 had 90000ms (90 seconds). With 15–45s reloads, seenWallet (in-memory
// only) resets on every reload — so the 90s timer was often irrelevant.
// The real problem: if a close click fails or the page reloads mid-close,
// the button would be skipped for up to 90s on that same page load.
// Now set to 12s — enough to survive the confirm modal flow, but not so long
// that a failed close leaves the button ignored through multiple scan cycles.
const CLOSE_POSITION_COOLDOWN_MS = 12000;

// ── WALLET SCROLL SWEEP ─────────────────────────────────────────────────────
// Flutter Web scrolls its trade list via WheelEvent (mouse wheel simulation),
// NOT via scrollTop on a DOM element — scrollTop has no effect on Flutter canvas.
// The flt-semantics accessibility tree contains ALL trade card TEXT regardless
// of scroll position (used for body.innerText scans), but Copy/Copied BUTTONS
// only become clickable once Flutter has rendered that card into view.
//
// The sweep simulates mouse-wheel events on the flt-glass-pane so Flutter
// physically scrolls its list, making off-screen buttons clickable.
//
// WALLET_SCROLL_STEPS    — wheel ticks per step (~3 × deltaY 120 = ~1 card height)
// WALLET_SCROLL_PAUSE_MS — pause after each step for Flutter to render new cards
// WALLET_SCROLL_MAX_POS  — total positions to visit (35 positions covers ~35 cards)
const WALLET_SCROLL_STEPS       = 3;     // wheel ticks per step
const WALLET_SCROLL_PAUSE_MS    = 400;   // ms to wait after each step for render
const WALLET_SCROLL_MAX_POS     = 35;    // max scroll positions to visit
...
// Persisted state — stored in localStorage (shared by every tab on this
// origin, unlike sessionStorage which is isolated per tab). This is what
// lets the notifications tab and the wallet tab hand information to each
// other (e.g. "I just opened a trade with this stop-loss, please verify it").
...
// FIX v22.1: NEW localStorage key — tracks the last time we saw a "Close
// Position" button and acted on it. Persists across page reloads (unlike
// seenWallet which is in-memory only), so we don't re-fire a close that is
// already mid-confirm even if the page reloads between click and confirm.
const CLOSE_POSITION_TS_KEY = '__AM_V22_LAST_CLOSE_TS__';

// v22.2: In-memory flag — true while a scroll sweep is in progress so
// scanWallet() doesn't start a second sweep before the first finishes.
let walletScrollSweepActive = false;
// Tracks what the body text contained at the END of the last full sweep, so
// we can detect when new content has appeared (e.g. a "Close Position" button
// that was below the fold on the previous sweep).
...
// seenNotifs / pendingTrades persist across reloads (and tab closes, since
// they live in localStorage) so a refresh never re-processes a notification
// or silently drops a trade that was mid-flight when the page reloaded.
...
function loadSeenNotifs() {
  try {
    ...
    if (raw) {
      ...
      L(`Restored ${seenNotifs.size} seen notification keys`);
    }
  } catch {}
}

// Pending-trade queue: if a fresh notification arrives while we're mid-trade,
// it's queued here instead of processed immediately. Persisted so a reload
// during execution doesn't lose the queue.
...
// Restore pending trades on boot — re-queues anything saved within MAX_AGE_MINUTES
// and discards anything older (it would be rejected by the age filter anyway).
function loadPendingTrades() {
  try {
    ...
    if (restored > 0) {
      L(`loadPendingTrades: restored ${restored} pending trade(s)`);
      logAct('PENDING_RESTORED', `${restored} trade(s) re-queued after reload`);
    }
  } catch {}
}
...
// ── RISK STATE (shared across both tabs via localStorage) ─────────────────
...
// Manual safety brake: window._pauseOpens(30) stops new trades from being
// opened for 30 minutes (wallet tab keeps running close/update logic as
// normal). Call window._unpause() to lift it early.
...
function showEmergencyBanner(message) {
  ...
  // This banner is informational only — it does not pause anything.
...
function clearEmergencyBanner() {
  ...
  // Clear the "already beeped" guard so the next emergency beeps fresh
...
function unlock() {
  ...
  badge('🟢 Active');
  L('unlocked');
  // If we're not on the home page (e.g. we ended up on a /post/ detail page
  // after opening a trade), navigate home first — the boot sequence on the
  // next page load will pick up scanning and drain the pending queue.
  if (!onPage(HOME_PATH)) {
    window.location.href = HOME_URL;
    return;
  }
  // Already home — process any queued trades right away
...
function closeDone() {
  ...
  badge('🟢 Active');
  clearEmergencyBanner();
  // FIX v22.1: Clear the persisted close-position timestamp so the next
  // reload can immediately detect a new "Close Position" if one appears.
...
// Builds a dedup key from a notification label. The first 20 characters
// (usually the trader's handle) are kept as a prefix so that two different
// traders opening the same symbol/leverage/direction don't collide on the
// same key — the rest of the label has its "Xm ago" / "Xs ago" timestamp
// stripped so the key stays stable as the notification ages.
...
// Scans every open-position card on the wallet page and returns the one with
// the largest dollar loss — this is the card the Emergency Alert/Close logic
// acts on, so it needs to be the actual worst position, not just the first
// one that happens to appear in document order. Uses the same card-matching
...
function abortForWalletEmergency(reason) {
  ...
  // Beep once per emergency event — cleared by clearEmergencyBanner() once resolved,
  // so a reload while the same emergency is still active won't re-beep.
  if (!localStorage.getItem(EMERGENCY_BEEP_KEY)) {
    playAlert(1);
    try { localStorage.setItem(EMERGENCY_BEEP_KEY, String(Date.now())); } catch {}
    logAct('EMERGENCY_BEEP', 'Alert tone fired — silent until resolved');
  } else {
    logAct('EMERGENCY_SILENT', 'Already alerted for this emergency — silent');
  }
  badge('🚨 LOSS ALERT (watching)', '#991b1b');
  // No pause here — the wallet tab keeps watching so the trader has a chance
  // to close the position at a better price on their own.
...
function scanNotifs() {
  ...
  const addTrade = (el, label) => {
    ...
    if (age > MAX_AGE_MINUTES) {
      seenNotifs.add(key);
      saveSeenNotifs(); // persist so a reload doesn't re-process this notification
  ...
  if (isExec) {
    // Don't mark these as "seen" yet — only once we actually process them.
    // If the page reloads while isExec is true, an early seenNotifs entry
    // would permanently block the trade from ever being opened.
    unique.forEach(t => {
    ...
    savePendingTrades(); // persist the queue so a reload can't lose it
    return;
  }
  ...
  // Trades restored from a prior page load have el === null — re-find the
  // live DOM node by its stable key before clicking it.
  if (!target.el) {
...
function verifyPostOpenRiskIfNeeded() {
  ...
  // If this flag is more than 5 minutes old, the trade context it refers to
  // is too stale to act on safely — clear it without triggering anything.
  if (Date.now() - (st.ts || 0) > 5 * 60 * 1000) {
    setRiskState({ needWalletVerify: false });
    logAct('WALLET_VERIFY', 'needWalletVerify flag was stale (>5min) — cleared without action');
    return;
  }
...
// Finds every "Trade Updates (N)" group header on the wallet page, captures
// its live DOM element (so it can be clicked directly later — no text
// re-search needed), deduplicates overlapping Flutter accessibility nodes
// for the same visual element, and returns the groups sorted top-to-bottom
// so processing order matches what's on screen.
...
// Expands a group by clicking its pre-captured header element directly —
// the element was captured by findTradeUpdateGroups() at scan time, so no
// text re-search is needed (which previously always re-found the same node).
...
// Visible "Copy" buttons = unconfirmed trade updates waiting to be applied.
...
// Visible "Copied" buttons = updates already confirmed.
function findVisibleCopiedButtons() {
  return [...document.querySelectorAll('flt-semantics,div,span,button')]
    .filter(el => textOf(el) === 'Copied' && visible(el));
}

// Counts Copy/Copied buttons within a vertical window below a group's header,
// so buttons belonging to OTHER expanded groups aren't attributed to this one.
//   groupEl  — the Trade Updates header DOM element
//   windowPx — how far below the header to scan (default 400px)
...
function waitForGroupExpansion(group, onReady, onFail) {
  ...
  // Check BEFORE clicking — if the group is already expanded, clicking the
  // header again would collapse it instead of expanding it.
...
// Identifies which confirmation modal (if any) is currently on screen.
//
// Flutter renders modal overlays in their own subtree, which a curated
// flt-semantics/button selector can miss entirely — so this scans EVERY
// visible element's text for known modal keywords (Strategy A), plus a
// body.innerText fallback as a backstop (Strategy B). Logs whatever
// keywords it finds, which is handy for debugging if a modal type is
// ever misclassified.
//
// Returns: 'adjust' | 'position' | 'confirm' | 'close' | null
function detectModalType() {
  // Strategy A — scan every visible element for modal keywords
  ...
  // "Adjust Position Size" modals don't expose a title in Flutter's
  // accessibility tree — only "Confirm" and "Reset" buttons are visible.
  // "Reset" doesn't appear in any other Involio modal, so [Confirm, Reset]
  // with none of the other keywords is an unambiguous match for "adjust".
  if (hasConfirm && hasReset && !hasGoBack && !hasCancel && !hasExitPrice) {
    L('detectModalType: [Confirm+Reset] → "adjust"');
    return 'adjust';
  }
...
// Clicks one "Copy" button (scoped to a group's Y window via groupEl), waits
// for whatever confirmation modal Flutter shows in response, resolves it,
// then verifies the button flipped to "Copied" before moving on to the next
// one. Recurses via processSingleCopy(loop+1, ...) until maxLoops is reached
// or no Copy buttons remain.
function processSingleCopy(loop, maxLoops, groupEl, onDone) {
  ...
  // Poll for the resulting modal every 300ms (first check after 400ms, giving
  // Flutter a moment to start rendering). Give up after 6s and move on.
...
// Processes every "Trade Updates" group on the wallet page, one at a time,
// inside a single lock. Each group's DOM element was captured at scan time
// by findTradeUpdateGroups(), so headers are clicked directly with no text
// re-search, and Copy/Copied counts are scoped to each group's Y window.
function processGroupQueue(groups, groupIdx) {
  ...
  // Expand the pre-captured group header and wait for its rows to appear
...
// Handles every "Trade Updates (N)" group on the wallet page in one
// continuous lock cycle via processGroupQueue(), rather than processing one
// group, unlocking, and hoping the next scan catches the rest.
//
// FIX v22.1: Moved the "Close Position" visibility check ABOVE this function
// in scanWallet(). If a direct "Close Position" button is visible, we act on
// it immediately without entering the Trade Updates lock cycle, which
// previously could hold the lock for 20–90+ seconds blocking the close.
//
// Two paths:
//  - If any "Copy" button is already visible, process immediately — these
//    are confirmed-pending updates and should never be left sitting.
//  - If nothing is visible, some groups may still be collapsed with hidden
//    Copy buttons inside. We expand-and-check those too, but only once per
//    unique combination of group labels within a 20s window (compositeKey),
//    so an unchanged wallet page doesn't get re-expanded every scan.
function handleWalletTradeUpdates() {
  const groups = findTradeUpdateGroups();
  if (!groups.length) return false;

  // Highest-count group first, so the most-updated trade is handled first
  ...
  lock('UPDATING');
  logAct('UPDATING', `${groups.length} group(s): ${groups.map(g => g.label).join(', ')} | checking for hidden updates`);
  processGroupQueue(groups, 0);
  return true;
}
...
//   Returns true if a close was triggered; false if no trades exceed threshold.
//
...
// ── WALLET SCROLL SWEEP ENGINE ────────────────────────────────────────────────
//
// v22.3 — CORRECTED scroll approach for Flutter Web.
//
// WHY scrollTop DOESN'T WORK ON FLUTTER WEB:
//   Involio is built with Flutter Web. Flutter renders its UI on a <canvas>
//   element managed entirely by its own engine. The trade card list is a
//   Flutter ListView — scrolling it requires sending WheelEvent (mouse wheel)
//   events to the flt-glass-pane element, exactly as a real user would scroll
//   with their mouse wheel. Setting scrollTop on any DOM element has zero effect
//   on Flutter's internal list position.
//
// WHAT ABOUT body.innerText / flt-semantics TEXT?
//   Flutter's accessibility layer (flt-semantics nodes) DOES expose ALL trade
//   card TEXT regardless of scroll position — this is why body.innerText scans
//   for symbols, dollar losses, and percentages work fine for all trades.
//   BUT: the Copy/Copied/Close Position BUTTONS only become interactable once
//   Flutter has actually rendered those cards into the visible viewport.
//   So we need real scrolling to make those buttons clickable.
//
// HOW THE WHEEL-EVENT SWEEP WORKS:
//   1. First scroll to the TOP by sending many upward wheel events.
//   2. At each position, run all wallet action checks (Close Position, dollar
//      stop-loss, emergency, Trade Updates).
//   3. If any action fires, stop — the action sequence takes over.
//   4. Otherwise, send WALLET_SCROLL_STEPS downward wheel ticks and repeat.
//   5. After WALLET_SCROLL_MAX_POS positions (or when no more scroll happens),
//      end the sweep. The list stays at its final position — the next page
//      reload resets it to top anyway.
//
// VISIBILITY TO THE USER:
//   You WILL see the page physically scroll up and down during the sweep.
//   This is intentional and correct — it means the script is working.
//   The sweep only runs when there's nothing urgent already visible on screen.
//
function getFlutterGlassPane() {
  return document.querySelector('flt-glass-pane') || document.body;
}

// Fires one WheelEvent on the Flutter glass pane.
// deltaY > 0 = scroll DOWN (content moves up), deltaY < 0 = scroll UP.
...
// Scroll to the very top of the Flutter list by sending a large burst of
// upward wheel events. Returns a Promise that resolves when done.
function flutterScrollToTop() {
  return new Promise(resolve => {
    // Send 60 upward ticks — enough to reach top from any position
    for (let i = 0; i < 60; i++) {
      flutterWheel(-300);
    }
    // Give Flutter time to render the top of the list
    setTimeout(resolve, 500);
  });
}

// Runs the wallet action checklist at the current scroll position.
// Returns true if any action was fired (caller should stop the sweep).
function walletChecksAtCurrentPosition() {
  if (isExec) return true; // something else took over
...
// Main scroll sweep: scrolls the Flutter wallet list from top to bottom,
// running wallet checks at each position. Calls onActionFired() if something
// urgent is found mid-sweep, or onComplete() when the sweep finishes cleanly.
function runWalletScrollSweep(onActionFired, onComplete) {
  if (walletScrollSweepActive) {
    onComplete?.();
    return;
  }
  walletScrollSweepActive = true;
  logAct('SCROLL_SWEEP', `Starting top-to-bottom sweep (${WALLET_SCROLL_MAX_POS} positions × ${WALLET_SCROLL_STEPS} wheel ticks)`);
  L(`runWalletScrollSweep: starting — ${WALLET_SCROLL_MAX_POS} positions`);

  // Step 1: scroll to top first so we always sweep the full list
  flutterScrollToTop().then(() => {
    let pos = 0;

    function step() {
      if (isExec) {
        // Action took over mid-sweep
        walletScrollSweepActive = false;
        onActionFired?.();
        return;
      }
      if (pos >= WALLET_SCROLL_MAX_POS) {
        // Reached the end
        L(`runWalletScrollSweep: finished ${pos} positions — sweep complete`);
        walletScrollLastSeenBody = document.body.innerText || '';
        walletScrollSweepActive = false;
        onComplete?.();
        return;
      }

      // Run checks at the current scroll position
      if (walletChecksAtCurrentPosition()) {
        logAct('SCROLL_FOUND', `Action fired at scroll position ${pos}`);
        walletScrollSweepActive = false;
        onActionFired?.();
        return;
      }

      // Also check for Trade Update Copy buttons at this position
      // (handleWalletTradeUpdates locks — if it fires, treat as action found)
      if (!isExec && handleWalletTradeUpdates()) {
        logAct('SCROLL_FOUND', `Trade Updates found at scroll position ${pos}`);
        walletScrollSweepActive = false;
        onActionFired?.();
        return;
      }
      ...
      for (let i = 0; i < WALLET_SCROLL_STEPS; i++) {
        flutterWheel(300); // 300 = one Flutter list-item height approximately
      }
      pos++;

      // Wait for Flutter to render newly exposed cards, then check again
...
// ── CLOSE POSITION DETECTION — the most important wallet action ─────────────
//
// FIX v22.1 — This is the core fix for the "30-minute delay" bug.
//
// ROOT CAUSE ANALYSIS:
//
// In v22.0, "Close Position" detection was the LAST check in scanWallet()
// (priority 7), running AFTER Trade Updates (priority 4). When a trader closed
// a position (e.g. TAO), the wallet page showed a "Close Position" button.
// But if ANY "Trade Updates (N)" group was present — even collapsed with no
// actual Copy buttons — handleWalletTradeUpdates() would:
//   1. Lock the executor
//   2. Enter waitForGroupExpansion() (up to 6s per group × 2 retries = 12s)
//   3. Unlock, trigger a page reload (15–45s)
//   4. On reload, seenWallet (in-memory Set) is empty → same groups found
//      again → same lock cycle repeats
// This loop could spin for 30+ minutes before the close was ever attempted.
//
// Additionally, the `'recent-close'` cooldown was 90 seconds in seenWallet.
// Since seenWallet resets on every page reload (it's in-memory), the 90s
// timer was essentially irrelevant for cross-reload dedup — but it did block
// re-detection for 90s if the click fired on the same page load.
//
// FIXES APPLIED:
//
// 1. checkClosePosition() now runs BEFORE handleWalletTradeUpdates() in
//    scanWallet(). A visible "Close Position" button is the highest-urgency
//    wallet action and must never be blocked by Trade Updates processing.
//
// 2. Cooldown is stored in localStorage (CLOSE_POSITION_TS_KEY) with a
//    short window (CLOSE_POSITION_COOLDOWN_MS = 12s). This persists across
//    page reloads, correctly preventing a double-fire when the confirm modal
//    causes a reload, while resetting fast enough to catch the next close.
//
// 3. closeDone() now clears CLOSE_POSITION_TS_KEY on completion so any
//    subsequent "Close Position" on a fresh trade is never blocked.
//
// 4. STALE_MS reduced from 20min → 8min so a stuck wallet tab reloads
//    faster, escaping any edge-case where all guards fail.
//
function checkClosePosition() {
  const body = document.body.innerText || '';
  if (!body.includes('Close Position')) return false;

  // Check if we already acted on a Close Position recently (persisted across reloads)
  try {
    ...
    if (lastCloseTs && Date.now() - lastCloseTs < CLOSE_POSITION_COOLDOWN_MS) {
      // Still inside the cooldown window — the confirm modal is likely in flight
      // (e.g. the page reloaded mid-confirm). Don't re-fire.
      const remaining = Math.round((CLOSE_POSITION_COOLDOWN_MS - (Date.now() - lastCloseTs)) / 1000);
      L(`checkClosePosition: cooldown active (${remaining}s remaining) — skipping`);
      return false;
    }
  } catch {}
  ...
  // Persist the timestamp BEFORE clicking — if the confirm modal triggers a
  // reload, we need this in localStorage to prevent the next page load from
  // re-firing the close before the first one has finished.
  try { localStorage.setItem(CLOSE_POSITION_TS_KEY, String(Date.now())); } catch {}

  logAct('CLOSING', `Direct Close Position found — acting immediately`);
  L(`checkClosePosition: Close Position button found — locking and clicking`);
  lock('CLOSING');
  click(btn);
...
function scanWallet() {
  ...
  if (isExec) return;
  if (walletScrollSweepActive) return; // sweep is running — don't interrupt
  ...
  // ── Priority order ─────────────────────────────────────────────────────
  //
  // Pass A — "what's on screen RIGHT NOW" (fast, runs every scan tick):
  //   1. Dismiss hover menu
  //   2. Handle blocking modals
  //   3. Post-open SL verify
  //   4. Close Position visible on screen → act immediately
  //   5. Dollar stop-loss visible on screen → act immediately
  //   6. Emergency loss visible on screen → act immediately
  //   7. Trade Updates visible on screen → process Copy buttons
  //
  // Pass B — full wheel-event scroll sweep (runs when Pass A finds nothing):
  //   8. Simulate mouse-wheel scrolling from top to bottom of the trade list.
  //      At each scroll position Flutter renders the newly visible cards, then
  //      checks 4–7 are re-run. A close signal on a trade 30+ cards down
  //      is detected within one sweep (~WALLET_SCROLL_MAX_POS positions ×
  //      WALLET_SCROLL_STEPS ticks each = the full list). You will see the
  //      page visually scrolling while this runs — that is correct behavior.
  //
  // After the sweep completes with no action, the list stays at its final
  // scroll position until the next page reload resets it to the top.

  verifyPostOpenRiskIfNeeded();

  // ── Pass A: on-screen checks ────────────────────────────────────────────
  ...
  // Trade Updates on the CURRENT screen — process any Copy buttons visible
  // right now before starting a scroll sweep (catches the common case where
  // updates are visible without needing to scroll at all).
  if (handleWalletTradeUpdates()) return;

  // ── Pass B: full scroll sweep ────────────────────────────────────────────
  // Run once per scan cycle when nothing urgent was found on-screen.
  // The sweep walks the entire trade list top-to-bottom, checking for close
  // signals and loss alerts at every scroll position.
  // After the sweep completes, handleWalletTradeUpdates() is called once more
  // at the bottom scroll position to catch groups that were off-screen before.
  runWalletScrollSweep(
    // onActionFired — something was found mid-sweep; scanWallet will pick it
    // up on the next tick once the action sequence unlocks the executor.
    () => { L('scanWallet: scroll sweep found an action — handing off to executor'); },

    // onComplete — sweep finished with no urgent actions found
    () => {
      if (isExec) return; // something else locked during the sweep
      // One final Trade Updates check now that we're at the bottom of the list —
      // this catches groups on trades that were below the fold before the sweep.
      if (handleWalletTradeUpdates()) return;
      L('scanWallet: scroll sweep complete — wallet fully checked');
    }
  );
}
...
function scan() {
  try {
    ...
    W(`scan() error: ${e.message}`);
  }
}

// ── WATCHDOGS ──────────────────────────────────────────────────────────────
// Force-unlock if a trade execution has been "in progress" too long
setInterval(() => {
...
// Wrong-page guard — navigates back home if we drift onto an unexpected URL
// for too long. Skipped entirely while a trade is executing (isExec=true).
setInterval(() => {
...
// Stale reload — reduced to 8min in v22.1 (was 20min).
// This is the last-resort safety net if lastActivity stops being refreshed.
setInterval(() => {
...
//
//   This complements the Web Worker keepalive below:
...
// Each tab schedules its own random reload between RELOAD_MIN_S and
// RELOAD_MAX_S (inclusive). If the timer fires while a trade is executing or
// during the post-action cooldown, the reload is skipped and re-scheduled
// with a fresh random delay rather than reloading mid-trade.
...
// Fires scanNotifs() almost immediately when new nodes appear in the DOM,
// instead of waiting up to SCAN_MS for the next poll tick. Throttled to at
// most one scan per THROTTLE_MS with a trailing call, so Flutter's frequent
// internal re-renders (animations, scrolling) can't trigger dozens of
// full-page scans per second.
function startMutationWatcher() {
  ...
  const runScan = () => {
    ...
    try { scanNotifs(); } catch (e) { W(`mutation-triggered scan error: ${e.message}`); }
  };
  ...
  const observer = new MutationObserver(() => {
    ...
    if (now - lastRun >= THROTTLE_MS) {
      ...
      trailingTimer = setTimeout(() => { trailingTimer = null; runScan(); }, THROTTLE_MS);
    }
  });
  observer.observe(target, { childList: true, subtree: true, attributes: false, characterData: false });
  L('MutationObserver active — instant notification detection enabled (throttled)');
}

// Page Visibility API — Chrome's intensive timer throttling (60s+ intervals)
// for hidden tabs doesn't affect visibility events, so this fires an
// immediate scan as soon as the tab regains focus, regardless of how long
// it was hidden.
...
// Web Worker keepalive — a minimal worker using a Blob URL pings the main
// thread every 10 seconds, keeping this tab's JS event loop awake even when
// backgrounded (workers are exempt from the throttling applied to a
// suspended tab's own timers).
...
// Restore persisted state before the first scan runs
loadSeenNotifs();
loadPendingTrades();

// Restore open-pause state (set via window._pauseOpens, shared across tabs)
...
// FIX v22.1: On boot, check if a close-position action is still in the
// cooldown window from a previous page load. If the timestamp is too old
// (older than cooldown), clear it — it's stale and would block fresh closes.
try {
  const lastCloseTs = parseInt(localStorage.getItem(CLOSE_POSITION_TS_KEY) || '0', 10);
  if (lastCloseTs && Date.now() - lastCloseTs > CLOSE_POSITION_COOLDOWN_MS) {
    localStorage.removeItem(CLOSE_POSITION_TS_KEY);
    L('Boot: stale CLOSE_POSITION_TS_KEY cleared');
  } else if (lastCloseTs) {
    const remaining = Math.round((CLOSE_POSITION_COOLDOWN_MS - (Date.now() - lastCloseTs)) / 1000);
    L(`Boot: close-position cooldown in effect (${remaining}s remaining from prior page load)`);
  }
} catch {}

setTimeout(() => {
  enableAcc();
  startWebLocksKeepalive();   // prevent Chrome from discarding this tab
  startAuthWatchdog();        // detect + recover from session-expiry (401) errors
  startAntiThrottleWorker();  // prevent Chrome from throttling JS timers in background
  setTimeout(() => {
    scan();
    setInterval(scan, SCAN_MS);
    startMutationWatcher();   // instant scan on DOM changes (notifications tab only)
    scheduleReload();
    ...
    L(`v22.1 FIX: Close Position now priority #4 (before Trade Updates). Cooldown: ${CLOSE_POSITION_COOLDOWN_MS}ms (localStorage-persisted). STALE_MS: ${STALE_MS / 60000}min.`);
    L(`v22.3 NEW: Wallet scroll sweep uses WheelEvent (visible page scroll). Positions: ${WALLET_SCROLL_MAX_POS}, Ticks/pos: ${WALLET_SCROLL_STEPS}, Pause: ${WALLET_SCROLL_PAUSE_MS}ms. You WILL see the wallet page scroll during sweeps.`);
    L(`Trade Updates: visible Copy buttons processed immediately; collapsed groups also checked for hidden updates`);
