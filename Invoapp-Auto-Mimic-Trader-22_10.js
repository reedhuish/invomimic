...
function isMimicPosition(symbol) {
  // v22.10 FIX: null/empty symbol MUST return false — unknown symbol is NEVER
  // treated as a mimic position. Returning true was allowing checkClosePosition()
  // to close positions without knowing which one it was closing.
...
window._status = () => {
  ...
  console.log(`\nv22.10: NULL-SYMBOL guard active — checkClosePosition() ABORTS if symbol unknown.`);
...
function classifyCopyAction(modalBodyText) {
  ...
  // v22.10 FIX: Drop the extra requirement — if Exit Price is present but Position Size
  // is NOT, that alone is a close modal. Involio shows Position Size when adjusting a
  // trade; it's absent only when fully exiting. The old check required at least one of
  // (hasCloseButton||hasCloseSignal||hasZeroSize) which missed clean close modals that
  // only display the exit price line without explicit close-language.
  const hasExitPrice   = /\bExit Price\b/i.test(b);
  const hasPositionSz  = /\bPosition Size\b/i.test(b);
  const isCloseModal   = hasExitPrice && !hasPositionSz; // Exit Price alone without Position Size = SKIP
...
function processSingleCopy(loop, maxLoops, groupEl, onDone) {
  ...
  function pollForModal() {
    ...
    // Secondary guard: classifyCopyAction() APPROVED this copy, but detectModalType()
    // says 'close' — classification conflict. Dismiss without confirming.
    // v22.10 FIX: The old code set urgentCloseSignalTs=Date.now() here, which caused
    // checkClosePosition() to immediately fire on the next scan and execute the close
    // anyway — completely negating the dismissal. Fixed: dismiss and CONTINUE the loop.
    // Only a real "Close Position" button on the wallet card triggers a legitimate close.
    if (modalType === 'close') {
      W(`processSingleCopy: 'close' modal mid-Copy — REFUSING (classifyCopyAction conflict)`);
      logAct('COPY_CLOSE_GUARD',`Loop ${loop}: 'close' modal dismissed — NOT setting urgentCloseSignalTs`);
      dismissMenu();
      // Do NOT set urgentCloseSignalTs — that would undo the dismissal on the next scan.
      // If the trader truly closed, checkClosePosition() will detect the wallet card button.
      setTimeout(()=>processSingleCopy(loop+1, maxLoops, groupEl, onDone), 1500);
      return;
    }
...
function readSymbolFromCloseButton(btn) {
  ...
  // Walk UP the Flutter DOM tree — increase depth to 18 for deeper card nesting
  let el = btn;
  for (let i = 0; i < 18; i++) {
  ...
  // v22.10 FIX: PROXIMITY-BASED FALLBACK — scan elements near the specific button
  // by screen position. The old body.indexOf() fallback found the FIRST "Close Position"
  // in the entire page text, which was frequently the WRONG card's button.
  // When multiple trade cards are visible, each has its own Close Position button;
  // body.indexOf returns card #1's symbol even when we clicked card #3's button.
  // This proximity search finds elements ABOVE the button within 400px — those
  // belong to the same card, giving the correct symbol.
  ...
  for (const candidate of document.querySelectorAll('flt-semantics,flt-semantics-container,div,span')) {
    ...
    // Must be above the button (same card header) and within scan window
    if (r.bottom < btnY - SCAN_WINDOW_PX) continue;
    if (r.top > btnY + 60) continue;   // allow slightly below for same-row text
  ...
  if (best) return best;
  // v22.10: Return null if symbol cannot be determined — caller (checkClosePosition)
  // must SKIP the close rather than proceed blind. Never guess.
  return null;
}
function checkClosePosition() {
  ...
  // v22.10 FIX: HARD GUARD — if symbol cannot be identified, NEVER execute the close.
  // Previously the if(symbol){...} block was SKIPPED when symbol was null, causing
  // the code to fall through and close whatever "Close Position" button was found —
  // which is how WLD (-$6.76) and ETH (-$8.38) were closed while the trader still had
  // their positions open. readSymbolFromCloseButton() can fail when the Flutter DOM is
  // flat/unparseable; we must treat that as "do not close" not "go ahead and close".
  if (!symbol) {
    W(`checkClosePosition: ⛔ ABORT — could not read symbol from Close Position button. Refusing to close an unknown position.`);
    logAct('CLOSE_GUARD_NULL', `symbol=null — skipped. urgentCloseSignalTs set for re-sweep.`);
    urgentCloseSignalTs = Date.now(); // Keep urgent so next sweep tries again with fresh DOM state
    return false;
  }

  if (!isMimicPosition(symbol)) {
    W(`checkClosePosition: ⚠️ SKIP — ${symbol} not in mimic registry`);
    logAct('CLOSE_GUARD', `Skipped ${symbol} — not in registry`);
    return false;
  }

  L(`checkClosePosition: ✅ ${symbol} confirmed in registry — closing`);
  urgentCloseSymbol = symbol;
  urgentCloseSignalTs=Date.now();
  try { localStorage.setItem(CLOSE_POSITION_TS_KEY,String(Date.now())); } catch {}
  logAct('CLOSING',`Close Position on ${symbol} ${direction||''} at ${new Date().toLocaleTimeString()}`);
  L(`checkClosePosition: 🟢 clicking Close Position NOW (${symbol} ${direction||'?'})`);
  lock('CLOSING');
  click(btn);
...
setTimeout(()=>{
  ...
  setTimeout(()=>{
    ...
    L(`════════════════════════════════════════════════════════`);
    L(`v22.10 DEEP-FIX: 5 root-cause bugs fixed — WLD -$6.76 autopsy`);
    L(`PRIMARY: checkClosePosition() was closing with NULL symbol (unknown card).`);
    L(` FIX 1: isMimicPosition(null) now returns FALSE — null = not a mimic`);
    L(`        (was TRUE — let null bypass the entire registry guard)`);
    L(` FIX 2: checkClosePosition() now HARD-ABORTS when symbol=null`);
    L(`        (was skipping if(symbol) block and executing close blindly)`);
    L(` FIX 3: readSymbolFromCloseButton() uses proximity search fallback`);
    L(`        (body.indexOf found FIRST Close Position = WRONG card symbol)`);
    L(` FIX 4: classifyCopyAction() isCloseModal = hasExitPrice && !hasPositionSz`);
    L(`        (simplified — Exit Price alone without Position Size = SKIP)`);
    L(` FIX 5: processSingleCopy 'close' guard no longer sets urgentCloseSignalTs`);
    L(`        (old: dismiss modal → immediately trigger close scan = self-defeating)`);
    L(`════════════════════════════════════════════════════════`);
    L(`v22.9 RETAINED: classifyCopyAction SKIP/FLIP/COPY, modal guard, registry,`);
    L(`flip detection, cross-card regex, close verification, auth watchdog.`);
